...
public class ScaleAnimationImageView extends ImageView {
    ...
    private float dip;
    private float[] mTapPoint;
    private PointColorListener mListener;
    ...
    public ScaleAnimationImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mPaint.setStrokeWidth(2 * dip);
        mPaint.setStyle(Paint.Style.STROKE);
    }

    public void setPointColorListener(PointColorListener listener) {
        mListener = listener;
    }
    ...
    protected void onDraw(android.graphics.Canvas canvas) {
        ...
        if (null != mTapPoint) {
            ...
            canvas.drawCircle(mTapPoint[0], mTapPoint[1], 10 * dip, mPaint);
            mPaint.setColor(Color.BLUE);
            canvas.drawCircle(mTapPoint[0], mTapPoint[1], 8 * dip, mPaint);
        }
    }
...
//        }
        ...
        public boolean onSingleTapUp(MotionEvent e) {
            ...
            if (drawable instanceof BitmapDrawable) {
                ...
                float[] outPoint = new float[]{x, y};
                Matrix matrix = new Matrix();
                getImageMatrix().invert(matrix);
                matrix.mapPoints(outPoint);

                try {
                    int currColor = bitmap.getPixel((int) outPoint[0], (int) outPoint[1]);
                    if (mListener != null) {
                        mListener.update(currColor);
                    }
